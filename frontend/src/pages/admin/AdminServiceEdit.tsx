...
const AdminServiceEdit = () => {
  ...
  const { mutate: editService, isPending: loading } = useMutation({
    mutationFn: async (newService: typeof serviceData) => {
      const res = await axiosInstance.put(`/services/${id}`, newService);
      return res.data;
    },
      ...
      if (axios.isAxiosError(err)) {
        toast.error(
          err.response?.data?.message ||
            "Hizmet güncellenirken bir sorun oluştı"
        );
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-gray-600">Terapi hizmetlerinizi detaylandırın</p>
        </div>
      </div>
